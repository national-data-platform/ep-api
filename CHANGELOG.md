...
- **Enabling sign-in is a single yes or no.** The realm, the client and its secret are all knowable — the registration names the realm and creates the client, and the provider's host is the one `AUTH_API_URL` validates tokens against, since they must be the same provider — so the installer derives them instead of asking the operator to retype what it already has. The derived realm is checked against its discovery document before anything is written; one that does not answer leaves sign-in off with an explanation, rather than producing a login that starts and cannot finish.
