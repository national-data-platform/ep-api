...
### Backwards compatibility
- CI only. No application code changed, and the shipped bundle is unaffected.
...
### Backwards compatibility
- No runtime change: the API, the image contents, and the published image name and tags are all unchanged. The image is built for `linux/amd64`, the only platform that has ever been published. Publishing now depends on the `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN` repository secrets; without them the workflow fails at the login step, leaving the GitHub release itself unaffected.
