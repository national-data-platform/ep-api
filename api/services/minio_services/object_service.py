...
from api.models.minio_models import (
    ObjectInfo, ObjectListResponse, ObjectUploadResponse, 
    ObjectMetadataResponse, PresignedUrlResponse
)
...
async def upload_object(
    ...
    try:
        ...
        logger.error(f"Failed to upload object '{object_key}' to bucket '{bucket_name}': {str(e)}")
        raise
...
async def list_objects(bucket_name: str, prefix: Optional[str] = None) -> ObjectListResponse:
    """
...
async def get_object_metadata(bucket_name: str, object_key: str) -> ObjectMetadataResponse:
    """
    Get object metadata.
...
async def delete_object(bucket_name: str, object_key: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            if e.code == "NoSuchKey":
                raise create_s3_error(f"Object '{object_key}' does not exist in bucket '{bucket_name}'", "NoSuchKey")
            raise
...
async def generate_presigned_upload_url(
    ...
    try:
        ...
        logger.info(f"Generated presigned upload URL for '{object_key}' in bucket '{bucket_name}'")
...
async def generate_presigned_download_url(
    ...
    try:
        ...
        logger.info(f"Generated presigned download URL for '{object_key}' in bucket '{bucket_name}'")
        return PresignedUrlResponse(url=url, expires_in=expires_in)
