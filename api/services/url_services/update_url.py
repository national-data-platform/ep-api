...
async def patch_url(
    ...
    # Update resource fields with new values if provided
    updated_data = {
        "name": resource_name if resource_name is not None else resource["name"],
        "title": resource_title if resource_title is not None else resource["title"],
        "owner_org": owner_org if owner_org is not None else resource["owner_org"],
        "notes": notes if notes is not None else resource["notes"],
        "resources": resource["resources"],
        "extras": resource.get("extras", []),
    }
