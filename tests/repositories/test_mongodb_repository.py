...
def test_package_patch(mongodb_repo):
    ...
    patched = mongodb_repo.package_patch(
        id=created["id"],
        title="Patched Title"
    )
...
def test_package_search_query(mongodb_repo):
    ...
    assert any("apple" in r["name"].lower() or "apple" in r["title"].lower()
...
def test_duplicate_organization_name(mongodb_repo):
    ...
    mongodb_repo.organization_create(
        name="duplicate-org",
        title="First Org"
    )
