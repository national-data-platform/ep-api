...
class TestSearchDatasourceEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_search_datasource_success_with_all_params(self, mock_datasource_services):
    ...
    async def test_search_datasource_service_exception(self, mock_datasource_services):
        ...
        mock_datasource_services.search_datasource.side_effect = Exception("Database connection error")
    ...
    async def test_search_datasource_generic_exception(self, mock_datasource_services):
        ...
        mock_datasource_services.search_datasource.side_effect = ValueError("Invalid search parameters")
    ...
    @patch("api.routes.search_routes.get.datasource_services")
    @pytest.mark.asyncio
    async def test_search_datasource_with_special_characters(self, mock_datasource_services):
        """Test search with special characters in parameters."""
        mock_results = [{"id": "dataset1"}]
        mock_datasource_services.search_datasource.return_value = mock_results

        result = await search_datasource(
            dataset_name="test-dataset_v1.0",
